...
class SuppQueryBatchSampler(Sampler):
    ...
    def __iter__(self):
        if self.seqtask:
            # Note that it divides the dataset according to the label or class
            ...
            for cls, inds in inds_per_cls:
                """
                这部分代码有点问题，原始的代码只适用于omniglot数据集，因为它每一类下只有20个数据，如果数据的数量大于
                20的话，那么每一个batch的query的数量也会大于num_query_per_cls，需要注意设置self.num_batch的大小。
                """

                # take the first (shot*batch) images and then divide it into several batch
                random.shuffle(inds)
